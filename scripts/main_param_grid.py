...
from utils import split_labeled_unlabeled, save_results, knn_factory, random_forest_factory, linear_svc_factory, gaussianNB_factory, decision_tree_factory
...
def train_model(model_name, model_function, dataset_name, preprocess_function, labeled_size,
    ...
    for hyperparams in generate_hyperparameter_combinations(models_hyperparameters[model_name]):
        ...
        try:
            train_results, test_results = train_and_evaluate(model_name, hyperparams, dataset_name, model, X_labeled, y_labeled, X_unlabeled, X_test, y_test)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    models_hyperparameters = {
                ...
                ,'TSVM' : {
                    'max_iter' : [-1, 3, 10, 50, 100, 150, 300],
                    'kernel' : ['rbf', 'linear'],
                    'random_state' : 42
                }
                ,'LabelPropagation' : {
    ...
    labeled_size = 0.3
    folder_to_save_results = "experiments_results_all_parameters"
